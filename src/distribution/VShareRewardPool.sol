...
contract VShareRewardPool is ReentrancyGuard {
  ...
  struct PoolInfo {
    ...
    uint256 accVSharePerShare; // Accumulated GHOGs per share, times 1e18. See below.
  ...
  constructor(address _ghog, address _devFund, uint256 _poolStartTime) {
  ...
  modifier onlyOperator() {
    require(operator == msg.sender, "GHogRewardPool: caller is not the operator");
    _;
  }
  ...
  function checkPoolDuplicate(IERC20 _token) internal view {
    ...
    for (uint256 pid = 0; pid < length; ++pid) {
      require(poolInfo[pid].token != _token, "GHogRewardPool: existing pool?");
    }
  }
  ...
  function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
    ...
    for (uint256 i = 0; i <= totalWeeks; i++) {
      ...
      totalRewards = totalRewards.add((weekEnd - weekStart).mul(weekRate));
    }
  ...
  function pendingShare(uint256 _pid, address _user) public view returns (uint256) {
    ...
    uint256 tokenSupply = pool.gauge != address(0)
      ? ISwapxGauge(pool.gauge).balanceOf(address(this))
    ...
    if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
      ...
      accVSharePerShare = accVSharePerShare.add(_ghogReward.mul(1e18).div(tokenSupply));
    }
  ...
  function updatePool(uint256 _pid) public {
    ...
    if (totalAllocPoint > 0) {
      ...
      uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
      uint256 _ghogReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
      pool.accVSharePerShare = pool.accVSharePerShare.add(_ghogReward.mul(1e18).div(tokenSupply));
    }
    pool.lastRewardTime = block.timestamp;
  }
  ...
  function getTotalEmittedSharesBetween(
    uint256 _fromTime,
    uint256 _toTime
