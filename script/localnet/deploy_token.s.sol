...
contract AnvilScript is Script {
  ...
  function run() public {
    ...
    bytes32 slot = keccak256(abi.encode(DEPLOYER, uint256(0)));
    vm.store(WS, slot, bytes32(uint256(50 ether)));

    // _valhalla = new Valhalla();
    // _valhalla.approve(SHADOW_ROUTER, 49 ether);
    IWETH(payable(WS)).approve(SHADOW_ROUTER, 49 ether);

    IRouter router = IRouter(payable(SHADOW_ROUTER));
    IRouter.route[] memory path = new IRouter.route[](1);
    path[0] = IRouter.route({ from: WS, to: OS, stable: false });
